...
export default class MessageReactionAddEvent {
    static async manuallyValidatePayment(message: MessageReaction | PartialMessageReaction) {
        ...
        if (!message.message.author) {
            createLogMessage({
                title: "MessageReactionAddEvent.manuallyValidatePayment",
        ...
        if (!reactioneeMember) {
            createLogMessage({
                title: "MessageReactionAddEvent.manuallyValidatePayment",
                message: "Reactionee member not found"
            })
            return
        }
        ...
        if (ownerId !== reactionerId) {
            createLogMessage({
                title: "MessageReactionAddEvent.manuallyValidatePayment",
                message: "Reactioner is not owner"
            })
            return
        }
        ...
        if (!unpaidIds.includes(reactioneeMember.user.id)) {
            createLogMessage({
                title: "MessageReactionAddEvent.manuallyValidatePayment",
                message: "Reactionee is not in unpaid list"
            })
            return
        }
